...
local action_save, action_save_global_key, action_jump, action_delete, which_find, fzf_find, fzf_find_for_rename, fzf_history
...
-- Parse keymap.toml to find existing key combinations
local function get_existing_keybindings()
  local content = read_keymap_file()
  if not content then return {} end

  local bindings = {}
  -- Match patterns like: { on = ["g", "h"], run = ... }
  for keygroup, key in content:gmatch('on%s*=%s*%[%s*"([^"]+)"%s*,%s*"([^"]+)"%s*%]') do
    if not bindings[keygroup] then
      bindings[keygroup] = {}
    end
    bindings[keygroup][key] = true
  end

  return bindings
end

-- Global key bookmark saver with two-step input
action_save_global_key = function(path, is_temp)
  if path == nil or #path == 0 then return end
  ...
  -- Get existing keybindings from keymap.toml
  local existing_bindings = get_existing_keybindings()

  -- Step 2: Get keygroup with ability to go back
  local keygroup = ""
  while true do
    local value, event = ya.input({
      title = "Enter keygroup ⟨e.g. g, b, P⟩",
      value = keygroup,
      pos = { "top-center", y = 3, w = 40 }
    })
    if event ~= 1 then return end  -- User cancelled

    keygroup = value or ""
    if #keygroup == 0 then
      ya.notify { title = "Bookmarks", content = "Empty keygroup", timeout = 1, level = "info" }
    elseif #keygroup > 1 then
      ya.notify { title = "Bookmarks", content = "Keygroup must be a single character", timeout = 1, level = "warn" }
    else
      break
    end
  end

  -- Show existing keys in this keygroup
  if existing_bindings[keygroup] then
    local used_keys = {}
    for k, _ in pairs(existing_bindings[keygroup]) do
      table.insert(used_keys, k)
    end
    table.sort(used_keys)
    local keys_str = table.concat(used_keys, ", ")
    ya.notify {
      title = "Keygroup [" .. keygroup .. "] - Existing keys",
      content = keys_str,
      timeout = 5,
      level = "info"
    }
    ...
    ya.notify {
      title = "Keygroup [" .. keygroup .. "]",
      content = "No existing keys found",
      timeout = 2,
      level = "info"
    }
  end

  -- Step 3: Get key with backspace to go back
  local key_char = ""
  while true do
    local value, event = ya.input({
      title = "Enter key ⟨single char, Backspace to change keygroup⟩",
      value = key_char,
      pos = { "top-center", y = 3, w = 50 }
    })
    ...
    if event ~= 1 then
      -- Check if user wants to go back (event 0 is usually cancel/escape)
      -- Let's allow going back to keygroup selection
      if event == 0 then
        -- Go back to keygroup selection
        keygroup = ""
        while true do
          local value2, event2 = ya.input({
            title = "Enter keygroup ⟨e.g. g, b, P⟩",
            value = keygroup,
            pos = { "top-center", y = 3, w = 40 }
          })
          if event2 ~= 1 then return end  -- User cancelled completely
        ...
        key_char = ""  -- Reset key input
      else
        return  -- Cancel completely
      end
    else
      key_char = value or ""
      if #key_char == 0 then
        ya.notify { title = "Bookmarks", content = "Empty key", timeout = 1, level = "info" }
      elseif #key_char > 1 then
        ya.notify { title = "Bookmarks", content = "Key must be a single character", timeout = 1, level = "warn" }
      else
        -- Check for collision
        if existing_bindings[keygroup] and existing_bindings[keygroup][key_char] then
          ya.notify {
            title = "Bookmarks",
            content = "Key combination [" .. keygroup .. "," .. key_char .. "] already configured",
            timeout = 2,
            level = "error"
          }
          return  -- Cancel the bookmark save
        end
  ...
  -- Create the key as a table [keygroup, key_char]
  local global_key = {keygroup, key_char}

  -- Check for conflicts with existing bookmarks
  local conflict = false
  for _, item in pairs(all_bookmarks) do
    if item and item.key and item.path ~= path then
      if type(item.key) == "table" and #item.key == 2 then
        if item.key[1] == keygroup and item.key[2] == key_char then
          conflict = true
          break
        end
      end
  ...
  if temp_bookmarks and not conflict then
    for _, item in pairs(temp_bookmarks) do
      if item and item.key and item.path ~= path then
        if type(item.key) == "table" and #item.key == 2 then
          if item.key[1] == keygroup and item.key[2] == key_char then
  ...
  if conflict then
    ya.notify {
      title = "Bookmarks",
      content = "Bookmark key [" .. keygroup .. "," .. key_char .. "] already in use",
      timeout = 2,
      level = "warn"
    }
    return
  ...
  -- Save the bookmark
  if is_temp then
    set_temp_bookmarks(path, { tag = tag, path = path, key = global_key })
    ya.notify {
      title = "Bookmarks",
      content = '[TEMP] "' .. tag .. '" saved with key [' .. keygroup .. ',' .. key_char .. ']',
      timeout = 2,
      level = "info"
    }
  else
    set_bookmarks(path, { tag = tag, path = path, key = global_key })
    local user_bookmarks = get_state_attr("bookmarks")
    save_to_file(mb_path, user_bookmarks)
    ya.notify {
      title = "Bookmarks",
      content = '"' .. tag .. '" saved with key [' .. keygroup .. ',' .. key_char .. ']',
      timeout = 2,
      level = "info"
    }
  end
...
local save_active_group = function(group_name, bookmarks_dir)
  local active_file = get_active_group_file(bookmarks_dir)
...
local load_active_group = function(bookmarks_dir)
  local active_file = get_active_group_file(bookmarks_dir)
...
local switch_group = function(group_name)
  if not group_name or group_name == "" then
    ya.notify { title = "Bookmarks", content = "Invalid group name", timeout = 2, level = "error" }
    return false
  end
  ...
  local file = io.open(group_path, "r")
  if file ~= nil then
  ...
  ya.notify {
    title = "Bookmarks",
    content = "Switched to group: " .. group_name .. " (" .. #user_bookmarks .. " bookmarks)",
    timeout = 2,
    level = "info"
  }
  return true
...
local create_group = function(group_name)
  if not group_name or group_name == "" then
    ya.notify { title = "Bookmarks", content = "Invalid group name", timeout = 2, level = "error" }
    return false
  end
  ...
  if file ~= nil then
    file:close()
    ya.notify { title = "Bookmarks", content = "Group '" .. group_name .. "' already exists", timeout = 2, level = "warn" }
    return false
  end

  -- Create empty group file
  file = io.open(group_path, "w")
  if file == nil then
    ya.notify { title = "Bookmarks", content = "Failed to create group: " .. group_name, timeout = 2, level = "error" }
    return false
  end
  file:close()

  ya.notify { title = "Bookmarks", content = "Created group: " .. group_name, timeout = 2, level = "info" }
  return true
end

local delete_group = function(group_name)
  if not group_name or group_name == "" then
    ya.notify { title = "Bookmarks", content = "Invalid group name", timeout = 2, level = "error" }
    return false
  end

  -- Don't allow deleting the active group
  local active_group = get_state_attr("active_group")
  if group_name == active_group then
    ya.notify {
      title = "Bookmarks",
      content = "Cannot delete active group. Switch to another group first.",
      timeout = 2,
      level = "error"
    }
    return false
  ...
  local file = io.open(group_path, "r")
  if file == nil then
    ya.notify { title = "Bookmarks", content = "Group '" .. group_name .. "' does not exist", timeout = 2, level = "warn" }
    return false
  end
  file:close()
  ...
  ya.notify { title = "Bookmarks", content = "Deleted group: " .. group_name, timeout = 2, level = "info" }
  return true
end

local action_switch_group = function()
  ...
  -- Build fzf list with active group indicator
  local permit = ui.hide()
  local temp_file_path = os.tmpname()
  local temp_file = io.open(temp_file_path, "w")
  if not temp_file then
    ya.notify { title = "Bookmarks", content = "Failed to create temp file", timeout = 2, level = "error" }
    return
  end

  for _, group in ipairs(groups) do
    local marker = (group == active_group) and "* " or "  "
    temp_file:write(marker .. group .. "\n")
  end
  temp_file:close()

  local cmd = string.format(
    "fzf --prompt='Select Group > ' --height=40%% --layout=reverse --border < '%s'",
    temp_file_path
  )
...
local action_delete_group = function()
  local groups = list_groups()
  ...
  -- Build fzf list excluding active group
  local permit = ui.hide()
  local temp_file_path = os.tmpname()
  local temp_file = io.open(temp_file_path, "w")
  if not temp_file then
  ...
  for _, group in ipairs(groups) do
    if group ~= active_group then
      temp_file:write(group .. "\n")
    end
  end
  temp_file:close()

  local cmd = string.format(
    "fzf --prompt='Delete Group > ' --height=40%% --layout=reverse --border < '%s'",
    temp_file_path
  )
...
return {
  setup = function(state, options)
    -- Use .data/bookmarks/ directory for group-based bookmarks
    local default_dir = (ya.target_family() == "windows" and os.getenv("APPDATA") .. "\\yazi\\.data\\bookmarks") or
        (os.getenv("HOME") .. "/.config/yazi/.data/bookmarks")
    ...
    local saved_group = load_active_group(bookmarks_dir)
    ...
    save_active_group(state.active_group, bookmarks_dir)
    ...
    if type(legacy_path) == "string" and legacy_path ~= '' then
        state.path = legacy_path
    else
        state.path = bookmarks_dir .. path_sep .. state.active_group
    end
  ...
  entry = function(self, jobs)
    ...
    elseif action == "save_cwd_global_key" then
      action_save_global_key(get_current_dir_path(), false)
    elseif action == "save_global_key" then
      if is_hovered_directory() then
        action_save_global_key(get_hovered_path(), false)
      else
        ya.notify { title = "Bookmarks", content = "Selected item is not a directory", timeout = 2, level = "warn" }
      end
